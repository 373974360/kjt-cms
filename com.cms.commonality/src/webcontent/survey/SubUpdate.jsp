	...
	<body>
        <div class="nui-fit" style="padding-top:5px">
			<div id="form1" method="post">
				<input id="sub.surveyId" name="sub.surveyId" class="nui-hidden"/>
	    ...
	    <script type="text/javascript">
	        ...
	        function removeItem(item,id){
	        	...
	        	console.log(id);
	        	if(id!=undefined){
	        		var json = nui.encode({items : [{"id":id}]});
					$.ajax({
						url : "com.cms.commonality.SubService.deleteItems.biz.ext",
						type : 'POST',
						data : json,
						cache : false,
	        ...
	        function setData(data){
	        	data = nui.clone(data);
	        	var json = nui.encode({sub:data});
				$.ajax({
					url:"com.cms.commonality.SubService.getSub.biz.ext",
					type:'POST',
			         data:json,
			         cache:false,
			         contentType:'text/json',
	        ...
	        function SaveData() {
		        ...
		        $("input[name='itemId']").each(function(){
		            itemId_array.push($(this).val());
		        });
		        var items_json = "[";
		        for(var i=0;i<item_array.length;i++){
		        	if(i>0){
		        		items_json += ',{"itemName":"'+item_array[i]+'","sort":"'+(i+1)+'","id":"'+itemId_array[i]+'","surveyId":"'+$("input[name='sub.surveyId']").val()+'"}';
		        	}else{
		        		items_json += '{"itemName":"'+item_array[i]+'","sort":"'+(i+1)+'","id":"'+itemId_array[i]+'","surveyId":"'+$("input[name='sub.surveyId']").val()+'"}';
		        	}
		        }
		        ...
		        var json1 = nui.encode(data1);
	           	$.ajax({
	                url: "com.cms.commonality.SubService.updateSub.biz.ext",
	                type: 'POST',
	                data: json1,
	                cache: false,
	                contentType:'text/json',
