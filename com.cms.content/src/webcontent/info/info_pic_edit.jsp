	...
	<body>
	    ...
	    <script type="text/javascript">
	        ...
	        function setPicContent(json){
				...
				if(json.picArray.length>0){
					for(var i=0;i<json.picArray.length;i++){
						...
						if(i > 0 && i < json.picArray.length){
							removeHtml = "<a class='icon-upload' onclick=\"movePic("+i+",'up')\" style='padding-left:20px;cursor:pointer'>上移</a><br/>"+
							"<a class='icon-download' onclick=\"movePic("+i+",'down')\" style='padding-left:20px;cursor:pointer'>下移</a>";
						}
						if(i == json.picArray.length-1){
							removeHtml = "<a class='icon-upload' onclick=\"movePic("+i+",'up')\" style='padding-left:20px;cursor:pointer'>上移</a><br/>";
						}
						_html+="<tr id='picArray_"+i+"'><td width='200' id='pic_"+i+"'><img src='"+json.picArray[i].src+"' width='200' height='120'/><input type='hidden' name='picUrl' value='"+json.picArray[i].src+"'/></td>"+
						"<td valign='top' id='input_"+i+"'><div>标题：<input type='text' name='picTitle' class='nui-textbox nui-form-input' value='"+json.picArray[i].title+"'/></div>"+
						"<div>描述：<textarea name='picRemark' style='height:60px;width:100%;font-size:12px;'>"+json.picArray[i].remark+"</textarea></div></td>"+
						"<td><a class='icon-remove' onclick='removePic("+i+")' style='padding-left:20px;cursor:pointer'>移除</a><br/>"+
						"<a class='icon-add' onclick=setThumbUrl('"+json.picArray[i].src+"') style='padding-left:20px;cursor:pointer'>设为标题图</a><br/>"+removeHtml+"</td></tr>";
					}
				}
				_html+="</table>";
				$("#picArray").html(_html);
	        ...
	        function onView() {
		        ...
		        data.info.picContent = getPicContent();
