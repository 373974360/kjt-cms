...
public class IndexLuceneManager {
	...
	public static void initIndex() {
		File file = new File(IndexManager.indexDir());
		if (!file.exists()) {
			file.mkdir();
		}
		// 初始化IndexWriter
	...
	public static IndexWriter getIndexModifier(boolean isNew)
			...
			IOException {
		String indexDir = IndexManager.indexDir();
		if (dir == null) {
			// 创建Directory对象
		...
		// org.wltea.analyzer.lucene.IKAnalyzer(), isNew,
		// IndexWriter.MaxFieldLength.UNLIMITED);
		IndexWriter indexWriter = new IndexWriter(dir, new StandardAnalyzer(
				Version.LUCENE_30), isNew, IndexWriter.MaxFieldLength.UNLIMITED);
	...
	/*
	 * Field.Store.COMPRESS:压缩保存,用于长文本或二进制数据 Field.Store.YES:保存
	 * Field.Store.NO:不保存
	 * 
	 * Field.Index.NO:不建立索引 Field.Index.TOKENIZED:分词,建索引
