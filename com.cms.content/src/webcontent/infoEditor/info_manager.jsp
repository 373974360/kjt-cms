...
<script type="text/javascript">
	...
	var info_list_1  = { title: '草稿箱', path: '<%=request.getContextPath() %>/content/infoEditor/info_list.jsp?infoStatus=1' ,refreshOnClick:true};
	var info_list_3  = { title: '已发布', path: '<%=request.getContextPath() %>/content/infoEditor/info_list.jsp?infoStatus=3' ,refreshOnClick:true};
	var info_list_4  = { title: '已撤稿', path: '<%=request.getContextPath() %>/content/infoEditor/info_list.jsp?infoStatus=4' ,refreshOnClick:true};
	var info_list_5  = { title: '已删除', path: '<%=request.getContextPath() %>/content/infoEditor/info_list.jsp?infoStatus=5' ,refreshOnClick:true};
	var applicationtabs_map = {};
	applicationtabs_map = [info_list_3,info_list_4,info_list_1,info_list_5];
	...
	function refreshTab(){
		var tabs = nui.get("infotabs");
		var applicationtabs = applicationtabs_map;
		for(var i=0;i<applicationtabs.length;i++){
			var obj = applicationtabs[i];
			obj.url = setUrlParam(obj.path);
		}
